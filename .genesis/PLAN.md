...
- **Outcome:** Every stub is filled and the project ships with a production setup guide. Disputes & feedback become anchored append-only events (agent=hitl, event_type=dispute/feedback — no schema change, audit-visible via M14). Threat model doc written from what's actually built (injection guard, sandbox, RBAC, audit spine, budget guard, circuit breaker, HMAC). Structured JSON logging module. Routing advisor consumes M16's cost-drift signal. Dashboard gains drift + explain pages. SETUP.md documents the real-use-case deployment (GitHub App/webhook, env vars, worker, Docker).
- **Phase:** 19 (HITL partial) + 11 (threat model partial) + 10 (logging partial) + 16 (economics partial) + 2/17 (dashboard extras)
- **Files:** `backend/hitl/dispute.py`, `backend/hitl/feedback.py`, `backend/security/threat_model.md`, `backend/observability/logging.py`, `backend/economics/routing_advisor.py`, `backend/api/hitl_router.py` (+mount in main.py), `frontend/src/app/drift/page.tsx`, `frontend/src/app/reviews/[id]/explain/[fid]/page.tsx`, `tests/test_m18.py`, `SETUP.md`
- **Demo command:** `pytest tests/test_m18.py -q` && `npm run build` (frontend/)
- **Success criteria:** (1) record_dispute/record_feedback emit anchored events (agent=hitl) with finding_id/reason/helpful in payload, queryable via audit; (2) threat_model.md is a real doc (assets, trust boundaries, threats → mitigations mapped to shipped code); (3) get_logger emits parseable JSON lines with review_id in extra; (4) routing_advisor: no drift → step default, cost drift past threshold → cheaper suggestion, floor step → 'already cheapest' (pure, unit-tested); (5) /hitl dispute+feedback endpoints key-protected (401 without); (6) dashboard /drift + explain pages build + render; (7) SETUP.md covers real-use-case deployment.
- **Loops:** L1, L4
- **Skills:** canon + tdd + security-engineering + production-readiness
- **Token budget:** 60000
...
- 2026-08-04 · M18 done — The Partials: disputes/feedback as anchored append-only events (agent=hitl, dispute/feedback — no new tables, audit-visible), structured JSON logging (secrets masked), routing advisor (consumes M16 cost drift, pure/deterministic), threat model (14 threats → shipped mitigations), key-protected HITL API, dashboard /drift + explain pages, SETUP.md real-use-case guide (10 tests) · L4 VERIFY APPROVE round 1 · 197 tests total · **THE PROJECT SHIPS — every roadmap phase + every stub complete**
